...
describe("commercial output action prerequisites", () => {
  const base = {
    ...
    simulationProvenance: "site_weather" as const,
    source: {
      projectId: "project-1",
  ...
  it("enables BOM generation only with authoritative design and financial lineage", () => {
  ...
  it("keeps proposal generation blocked until a matching BOM exists", () => {
  ...
  it("keeps commercial actions blocked for a non-finalized design", () => {
    ...
    expect(readiness.blockers).toContain("Finalize the design before creating commercial outputs.");
  });
});
