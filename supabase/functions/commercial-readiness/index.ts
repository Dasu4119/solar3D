...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!design?.active_version_id) return out({ success: true, readiness: { designFinalized: false, engineeringAccepted: false, simulationCompleted: false, financialCompleted: false, bomAvailable: false, proposalAvailable: false, simulationProvenance: null, warnings: [], source: { projectId, designVersionId: "", simulationRunId: null, financialRunId: null, bomRunId: null, proposalRunId: null } } });
    ...
    return out({ success: true, readiness: {
      ...
      source: { projectId, designVersionId: version.id, simulationRunId: simulation?.id ?? null, financialRunId: financial?.id ?? null, bomRunId: bom?.id ?? null, proposalRunId: proposal?.id ?? null },
    } });
  } catch (error) {
