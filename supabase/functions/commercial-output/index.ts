...
async function authClient(req: Request) {
  const url = Deno.env.get("SUPABASE_URL"); const key = Deno.env.get("SUPABASE_ANON_KEY"); const authorization = req.headers.get("Authorization");
...
async function requireDesignAccess(db: any, userId: string, designVersionId: string) {
  ...
  if (design.active_version_id !== designVersionId || version.status !== "finalized") throw new Error("Commercial outputs require the active finalized design version");
...
async function nextRunNumber(db: any, table: string, column: string, id: string) {
  ...
  if (error) throw error; return Number(data?.run_number ?? 0) + 1;
}
...
async function buildBom(db: any, designVersionId: string) {
  const { data: layouts, error } = await db.from("panel_layouts").select("id,module_id,roof_id,panel_count,dc_capacity_kw").eq("design_version_id", designVersionId).order("created_at", { ascending: true });
  ...
  const items = layouts.map((layout: any) => {
    const module = byModule.get(layout.module_id); if (!module) throw new Error(`Module ${layout.module_id} is unavailable`);
    return { category: "module", sku: module.id, manufacturer: module.manufacturer, model: module.model, quantity: Number(layout.panel_count ?? 0), unitPowerWatts: Number(module.power_w), roofId: layout.roof_id, layoutId: layout.id };
...
Deno.serve(async (req) => {
  ...
  try {
    const { db, user } = await authClient(req); const body = await req.json();
    if (body.action === "generate_bom") {
      ...
      const result = await buildBom(db, version.id); const runNumber = await nextRunNumber(db, "bom_runs", "financial_run_id", financial.id);
      const input = { designVersionId: version.id, financialRunId: financial.id, sourceFinancialResultHash: financial.result_hash };
      const { data: bom, error } = await db.from("bom_runs").insert({ financial_run_id: financial.id, design_version_id: version.id, run_number: runNumber, status: "completed", engine_name: "solar3d-bom", engine_version: "2026.08.p1.3", input_snapshot: input, result_snapshot: result, source_financial_result_hash: financial.result_hash, created_by: user.id, completed_at: new Date().toISOString() }).select("*").single();
      if (error) throw error; return out({ success: true, bom });
    }
    if (body.action === "generate_proposal") {
      ...
      const { version, project } = await requireDesignAccess(db, user.id, body.design_version_id); const financial = await loadFinancial(db, body.financial_run_id, version.id);
      ...
      if (bom.status !== "completed" || bom.design_version_id !== version.id || bom.financial_run_id !== financial.id || bom.source_financial_result_hash !== financial.result_hash) throw new Error("BOM lineage does not match the active design and financial run");
      const input = { projectId: project.id, projectName: project.name, designVersionId: version.id, financialRunId: financial.id, bomRunId: bom.id, sourceFinancialResultHash: financial.result_hash, sourceBomResultHash: bom.result_hash };
      const result = { title: `${project.name ?? "Solar3D"} Solar Proposal`, designVersion: version.version_number, financial: financial.result_snapshot, bom: bom.result_snapshot };
      const runNumber = await nextRunNumber(db, "proposal_runs", "bom_run_id", bom.id);
      const { data: proposal, error } = await db.from("proposal_runs").insert({ bom_run_id: bom.id, financial_run_id: financial.id, design_version_id: version.id, run_number: runNumber, status: "completed", engine_name: "solar3d-proposal", engine_version: "2026.08.p1.3", input_snapshot: input, result_snapshot: result, source_bom_result_hash: bom.result_hash, source_financial_result_hash: financial.result_hash, created_by: user.id, completed_at: new Date().toISOString() }).select("*").single();
      if (error) throw error; return out({ success: true, proposal });
    }
    ...
    return out({ error: message }, message === "Unauthorized" ? 401 : message === "Access denied" ? 403 : 400);
  }
});
