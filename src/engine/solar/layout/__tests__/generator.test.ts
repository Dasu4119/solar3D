...
describe("generateLayoutCandidates", () => {
  it("uses canonical obstacle constraints instead of legacy layout obstacle logic", () => {
    ...
    expect(candidates.some((candidate) => candidate.placement.center.x > 3 && candidate.placement.center.x < 7)).toBe(false);
  });

  it("rejects candidates that violate a non-axis-aligned roof edge setback", () => {
    const region = buildUsableRoofRegion(
      [
        { x: 0, y: 0 },
        { x: 8, y: 0 },
