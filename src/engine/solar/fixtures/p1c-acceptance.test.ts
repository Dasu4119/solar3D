...
import { ApiDesignPersistence, encodeRoofGeometry, type DesignPersistenceSnapshot } from "@/shared/api/design-persistence";
...
describe("P1-C end-to-end 3D-to-production acceptance", () => {
  it("keeps the complete deterministic pipeline coherent", async () => {
    ...
    const rightRegion = buildUsableRoofRegion([...P1C_ROOF_REGIONS[1]], P1C_OBSTACLES, { edgeM: 0.5, obstacleClearanceM: 0.75 });
    const leftCandidates = generateLayoutCandidates(leftRegion.roof, P1C_PANEL, { allowedRotations: [0, 90] }, [], undefined, undefined, [leftRegion]);
    const rightCandidates = generateLayoutCandidates(P1C_ROOF_REGIONS[1], P1C_PANEL, { allowedRotations: [0, 90], obstacles: P1C_LAYOUT_OBSTACLES }, [], [{ outer: P1C_ROOF_REGIONS[1] }], undefined, [rightRegion]);
    const validPlacements = [...leftCandidates, ...rightCandidates].filter((candidate) => candidate.valid).map((candidate) => candidate.placement);
    ...
    const production = calculateAnnualProduction({ panelCount: validPlacements.length, panelPowerWatts: P1C_PANEL.powerWatts, ...P1C_PRODUCTION_INPUT });
    ...
    const persistence = new ApiDesignPersistence({
      async invoke(action, body) {
        if (action === "save_design") {
          ...
          const panelPlacements = (body.panel_placements ?? []) as DesignPersistenceSnapshot["panelPlacements"];
          const metrics = body.metrics as DesignPersistenceSnapshot["metrics"];
          const roofGeometry = (body.roof as { geometry: ReturnType<typeof encodeRoofGeometry> }).geometry;
          persisted.set(designId, { designId, roof: roofGeometry.mesh, roofPlanes: roofGeometry.planes, panelPlacements, metrics });
          return { success: true, design_version: { id: "p1c-version-1", metrics } };
        }
        if (action === "get_design") {
          const snapshot = persisted.get(String(body.design_id));
          return snapshot
            ? { success: true, active_version: { id: "p1c-version-1", metrics: snapshot.metrics }, roofs: [{ geometry: encodeRoofGeometry(snapshot.roof, snapshot.roofPlanes ?? []) }], panel_placements: snapshot.panelPlacements.map((placement) => ({ id: placement.id, x: placement.center.x, y: placement.center.y, rotation_degrees: placement.rotation })) }
            : { success: false };
        }
    ...
    const snapshot: DesignPersistenceSnapshot = {
      ...
      metrics: { panel_count: validPlacements.length, dc_capacity_kw: production.dcCapacityKwp, annual_kwh: production.annualKwh, monthly_kwh: production.monthlyKwh, shading_loss_pct: production.shadingLossPct, warnings: production.warnings },
    };
