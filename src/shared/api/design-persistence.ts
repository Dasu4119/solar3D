...
export function decodeRoofGeometry(geometry: unknown): { mesh: Point[]; roofPlanes: RoofPlane[] } {
  if (geometry && typeof geometry === "object" && (geometry as PersistedRoofGeometry).schemaVersion === 1) {
    const persisted = geometry as PersistedRoofGeometry;
    return { mesh: persisted.mesh, roofPlanes: persisted.planes };
  }
  return { mesh: Array.isArray(geometry) ? geometry as Point[] : [], roofPlanes: [] };
}
...
export class ApiDesignPersistence implements DesignPersistence {
  ...
  async load(designId: string): Promise<DesignPersistenceSnapshot | null> {
    ...
    const decoded = decodeRoofGeometry(persistedRoof?.geometry);
    const layout = response.panel_layouts?.[0];
    const moduleId = layout?.module_id ?? null;

    const panelPlacements = (response.panel_placements ?? []).map((placement) => ({
      id: placement.id ?? crypto.randomUUID(),
      // Module identity belongs to the persisted layout, not the editor's defaults.
      panelId: moduleId ?? "",
  ...
  async save(snapshot: DesignPersistenceSnapshot): Promise<DesignPersistenceSnapshot> {
    const response = await this.api.invoke("save_design", {
      ...
      roof: {
        ...(snapshot.roofId ? { id: snapshot.roofId } : {}),
        geometry: encodeRoofGeometry(snapshot.roof, snapshot.roofPlanes ?? []),
        area_m2: snapshot.roofAreaM2 ?? null,
      },
...
export function createAuthenticatedDesignPersistence(): DesignPersistence {
  return new ApiDesignPersistence({
    invoke: (action, body) => invokeFunction("solar-project-api", { action, ...body }),
  });
}
