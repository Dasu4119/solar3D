...
export function FinancialSummary({ metrics }: { metrics?: FinancialSummaryMetrics }) {
  ...
  return (
    <section aria-label="financial summary" data-testid="solar-financial-summary">
      ...
      {!configured ? <p data-testid="financial-not-configured">Financial inputs are not configured yet. No financial value is fabricated.</p> : <>
      ...
      {warnings.length > 0 && <div role="note" style={{ marginTop: 8, fontSize: 12 }}>{warnings.map((warning) => <div key={warning}>{warning}</div>)}</div>}
    </section>
  );
}
