...
const tools: { id: DesignTool; label: string; hint: string }[] = [
  { id: "select", label: "Select", hint: "V" }, { id: "roof", label: "Roof", hint: "R" },
  { id: "panel", label: "Panel", hint: "P" }, { id: "obstacle", label: "Obstacle", hint: "O" }, { id: "measure", label: "Measure", hint: "M" },
];
const SCALE = 55;
const ORIGIN = { x: 120, y: 70 };
function worldToSvg(point: Point2D, maxY: number) { return { x: ORIGIN.x + point.x * SCALE, y: ORIGIN.y + (maxY - point.y) * SCALE }; }
...
export function DesignWorkspace({ projectId }: { projectId: string }) {
  ...
  const panelById = useCallback((id: string) => id === module?.id && module ? ({ id: module.id, manufacturer: module.manufacturer, model: module.model, widthM: module.widthM, lengthM: module.lengthM, powerWatts: module.powerWatts, efficiency: module.efficiency } satisfies SolarPanelSpec) : undefined, [module]);
  ...
  const pointerToWorld = useCallback((event: React.PointerEvent<SVGSVGElement>) => { const rect = stageRef.current?.getBoundingClientRect(); return rect ? svgToWorld(event.clientX, event.clientY, rect, maxY) : null; }, [maxY]);

  const onPointerMove = useCallback((event: React.PointerEvent<SVGSVGElement>) => {
    const point = pointerToWorld(event); if (!point) return; setCursor(point);
    if (draggingId) { const current = panels.find((panel) => panel.id === draggingId); if (!current) return; const validation = getPreview(point, current.rotation as 0 | 90 | 180 | 270, draggingId); if (validation?.valid) movePanel(draggingId, point.x, point.y); }
  }, [draggingId, getPreview, movePanel, panels, pointerToWorld]);
  const onStageClick = useCallback(() => { if (!module || activeTool !== "panel" || !preview?.valid || draggingId) return; const placement = preview.placement; addPanel({ id: `panel-${Date.now()}`, x: placement.center.x, y: placement.center.y, rotation: placement.rotation }); }, [activeTool, addPanel, draggingId, module, preview]);
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      const snapshot = await persistence.save({
        designId: context.designId, roof, roofId: context.roofId, roofAreaM2: context.roofAreaM2, moduleId: module.id,
      ...
      <div className="design-body">
        ...
        <section className="cad-stage" onPointerLeave={() => setCursor(null)}>
          <svg ref={stageRef} viewBox="0 0 800 500" width="100%" height="100%" role="application" aria-label="Solar design canvas" onPointerMove={onPointerMove} onClick={onStageClick}>
            ...
            <rect width="800" height="500" fill="url(#solar-grid)" /><polygon points={roofPoints} fill="currentColor" fillOpacity="0.04" stroke="currentColor" strokeWidth="2" />
            <text x={ORIGIN.x + 10} y={ORIGIN.y + 20} fontSize="13">Roof area · {context.roofAreaM2 == null ? "—" : `${context.roofAreaM2.toFixed(0)} m²`}</text>
