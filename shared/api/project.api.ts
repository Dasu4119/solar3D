...
function mapProject(row: ProjectRow): Project {
  return { id: row.id, organizationId: row.organization_id, customerId: row.customer_id, name: row.name, status: row.status };
}

function mapSite(row: SiteRow): Site {
  return { id: row.id, projectId: row.project_id, name: row.name, latitude: row.latitude, longitude: row.longitude };
}
